...
class LocalStorageService {
  ...
  // --- Simpan User di List (Fitur Save Dosen) ---
  ...
  Future<void> addUserToSavedList({
    ...
    // Cek duplikasi userId biar nggak disave 2 kali
  ...
  Future<void> saveMahasiswaToLocal({required String id, required String nama}) async {
    final prefs = await SharedPreferences.getInstance();
    final rawList = prefs.getStringList(_savedMahasiswaKey) ?? [];

    final isDuplicate = rawList.any((item) => (jsonDecode(item) as Map)['id'] == id);
  ...
  Future<void> removeSavedMahasiswa(String id) async {
    final prefs = await SharedPreferences.getInstance();
    final rawList = prefs.getStringList(_savedMahasiswaKey) ?? [];
    rawList.removeWhere((item) => (jsonDecode(item) as Map)['id'] == id);
    await prefs.setStringList(_savedMahasiswaKey, rawList);
  }
